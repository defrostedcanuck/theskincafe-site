...
| Client-owned infra from day one | No platform lock-in; clean handoff; no ongoing hosting fees paid to agency | **REVISED 2026-04-20:** Build on Craig's Vercel/Resend/Cloudflare during Milestone 1; migrate everything to client-owned accounts at Phase 4 launch cutover. Lowers client friction upfront; Phase 1 plans 02/03/04 scope moves to Phase 4. |
| Build-on-Craig, migrate-at-launch | Client doesn't want to juggle account setup during build. Craig hosts on his own Vercel team, uses his own Resend sandbox, and only requires client-owned creds at launch. Square production token is the sole exception — needed at launch for real leads to hit the client's Marketing list; Craig uses his own Square sandbox during dev. | Confirmed 2026-04-20 |
